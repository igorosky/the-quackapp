...
struct MediaView: View {
    ...
    init(
        title: String,
        items: [String],
        mediaType: DuckDetailView.MediaType,
        startIndex: Int = 0
    ) {
        self.title = title
        self.items = items
            ...
            VStack(spacing: 20) {
                ...
                if items.isEmpty {
                    RoundedRectangle(cornerRadius: 16)
                        ...
                        .frame(height: 220)
                        .overlay(
                            Text("No media available").foregroundColor(
                                .secondary
                            )
                        )
                } else {
                    ...
                    let insertion: AnyTransition = .move(
                        edge: lastAction >= 0 ? .trailing : .leading
                    ).combined(with: .opacity)
                    let removal: AnyTransition = .move(
                        edge: lastAction >= 0 ? .leading : .trailing
                    ).combined(with: .opacity)
                ...
                HStack(spacing: 40) {
                    ...
                    let nextEnabled =
                        !(items.isEmpty || index >= items.count - 1)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 20).fill(
                            prevEnabled
                                ? Theme.buttonGreen : Color.gray.opacity(0.35)
                        )
                    )
                    .foregroundColor(prevEnabled ? .white : .secondary)
                    ...
                    .foregroundColor(nextEnabled ? .white : .secondary)
                }
